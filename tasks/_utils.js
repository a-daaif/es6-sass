...
var spawn = require('child_process').spawn,
     ...
     *   }
        ...
        listFiles(thisPath, mustDelete) {
            ...
            if (fs.existsSync(thisPath)) {
                ...
                tmpFiles.forEach((file) => {
                    var curPath = thisPath + path.sep + file
                    files.push(curPath)
        ...
        resolve(projectPath, esskOptions) {
            ...
            esskOptions.scssBasePath = projectPath + path.sep + esskOptions.scssBasePath.replace('/', path.sep)
            esskOptions.jsDest = projectPath + path.sep + esskOptions.jsDest.replace('/', path.sep)
            esskOptions.cssDest = projectPath + path.sep + esskOptions.cssDest.replace('/', path.sep)
            esskOptions.projetPath = projectPath;
            return esskOptions
        },
        getDependencies() {
